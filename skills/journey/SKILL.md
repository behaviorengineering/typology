...
description: >-
  ...
  write architecture/typology-journey.md from the plan template, discover a
...
| Artefact | Path |
|----------|------|
| Journey (session) | `REPO/architecture/typology-journey.md` |
| Draft catalog | `REPO/architecture/typology.draft.yaml` |
| Confirmed catalog | `REPO/architecture/typology.yaml` |

## When to load

- Typology just landed in a repo (first catalog, onboard, first map)
- `architecture/typology-journey.md` exists and Status.phase is not `done`
...
- MUST: copy [plan-template.md](plan-template.md) to `architecture/typology-journey.md` when that file is missing and this skill loaded for a first map or refine
...
Enforcement: `architecture/typology-journey.md` exists; Resume.phase matches the next pending checkbox
Violation: STOP, create or repair the journey file from the template, copy any known decisions into it, then continue

CORRECT: first turn writes `architecture/typology-journey.md` with Status.phase `land`; next chat Reads it and continues at the first pending slice
...
- MUST: `typology discover REPO --out REPO/architecture/typology.draft.yaml` when there is no confirmed catalog and no draft
- MUST: if `architecture/typology.yaml` already exists, copy it to the draft path when the draft is missing, then skip discover unless the operator explicitly asked to rediscover
- MUST NOT: `typology discover REPO` with the default out path when `architecture/typology.yaml` exists
- MUST NOT: write `architecture/typology.yaml` before Status.phase is `commit`

Enforcement: draft path in the discover command; git status of `architecture/typology.yaml` unchanged through phases `land` .. `desired`
...
CORRECT: `typology discover . --out architecture/typology.draft.yaml`
...
- [ ] **Journey file:** `architecture/typology-journey.md` exists and Resume matches this turn
...
- [ ] **Draft path:** discover or catalog copies used `architecture/typology.draft.yaml`
