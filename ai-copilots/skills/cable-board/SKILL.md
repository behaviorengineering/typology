...
description: >-
  ...
  wiring graph (CLI: typology assembly-graph), interpret hubs, leaves, roles,
  import cables, and wrong-way edges, and steer AI implementation so new
  imports stay intentional. Load when the operator says cable board, assembly
  graph, package wiring, import cables, wrong-way imports, hubs/leaves, or
  asks to implement against observed wiring.
---
...
**CLI command (stable id):** `typology assembly-graph` · **Default artefact:** `tmp/typology/assembly-graph.json` · **Library:** `assemblygraph/` · **Catalog skill:** [catalog/SKILL.md](../catalog/SKILL.md) · **CLI skill:** [cli/SKILL.md](../cli/SKILL.md) · **First map:** [journey/SKILL.md](../journey/SKILL.md)

Human and agent name: **cable board**. File and command names stay `assembly-graph` until a deliberate rename ships.

## When to load

- Operator says cable board, assembly board, assembly graph, or package wiring
...
| Term | Meaning |
|------|---------|
| Cable board | The portable wiring document (JSON) plus the practice of reading and implementing against it |
...
3. **Read the board:** open the JSON. Inventory hubs, leaves, and every edge with `wrongWay: true` (read `wrongWayReason`).
4. **Align with catalog:** if `.typology/typology.yaml` exists, map package paths to slice `owns` / `surfaces` / `libraries`. Cables that cross slices MUST already have (or gain) a matching `sliceBindings` row per [catalog/SKILL.md](../catalog/SKILL.md).
5. **Implement:** when adding imports, prefer cables that stay inward on role layer and that match declared bindings. After the change, regenerate the board and re-check wrong-way count.
6. **Debt:** if a wrong-way cable must remain, record it in `.typology/typology-journey.md` under technical debt (or the consumer's agreed debt log) with target refactoring. MUST NOT silently accept new wrong-way cables.
